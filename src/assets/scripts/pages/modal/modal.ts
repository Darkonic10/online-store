...
export function addModal(container: HTMLDivElement, button: HTMLButtonElement): void {
  ...
  const detailsHead = createElementWithOptions('h2', HTMLHeadingElement, {
    innerText: 'Personal details',
    className: 'modal__detail-head',
  });
  ...
  const inputName = createElementWithOptions('input', HTMLInputElement, {
    type: 'text',
    placeholder: 'Name',
    className: 'input modal__input-name',
  });
  const inputPhone = createElementWithOptions('input', HTMLInputElement, {
    type: 'tel',
    placeholder: 'Phone number',
    min: '9',
    className: 'input modal__input-phone',
  });
  const inputAddress = createElementWithOptions('input', HTMLInputElement, {
    type: 'text',
    placeholder: 'Delivery address',
    className: 'input modal__input-address',
  });
  const inputEmail = createElementWithOptions('input', HTMLInputElement, {
    type: 'email',
    placeholder: 'E-mail',
    className: 'input modal__input-email',
  });

  const cardDetails = createElementWithOptions('div', HTMLDivElement, { className: 'modal__card-details' });
  const cardHead = createElementWithOptions('h2', HTMLHeadingElement, {
    className: 'modal__card-head',
    innerText: 'Credit card details',
  });
  ...
  const inputCardNumber = createElementWithOptions('input', HTMLInputElement, {
    type: 'number',
    placeholder: 'Card number',
    className: 'modal__card-input-number',
  });
  ...
  const validText = createElementWithOptions('span', HTMLSpanElement, {
    innerText: 'VALID:',
    className: 'modal__valid-text',
  });
  const inputValid = createElementWithOptions('input', HTMLInputElement, {
    type: 'text',
    placeholder: 'Valid date',
    className: 'modal__valid-input',
  });

  const cvvData = createElementWithOptions('div', HTMLDivElement, { className: 'modal__other-cvv' });
  const cvvText = createElementWithOptions('span', HTMLSpanElement, {
    innerText: 'CVV:',
    className: 'modal__cvv-text',
  });
  const cvvValid = createElementWithOptions('input', HTMLInputElement, {
    type: 'number',
    placeholder: 'Code',
    className: 'modal__cvv-input',
  });
  ...
  const confirmButton = createElementWithOptions('button', HTMLButtonElement, {
    innerText: 'CONFIRM',
